$( document ).ready(function() {
	...
	/* BEGIN Overrides by Andy @ Direct Design */
	if ($(window).width()>=1024 && $('#mysidebar').length) {
		var menu = $('#mysidebar');
		var dims = menu[0].getBoundingClientRect();
		var tfp = $('.sidebarTitle').outerHeight(true)+$('.navTitle').outerHeight(true); // top fix position
		var wh = $(window).height(); // viewport height
		var ft = $('.site-footer')[0].getBoundingClientRect().top; // current footer top
		...
		menu.css({
			...
			overflow:'scroll',
			bottom:(wh-ft)});
		// adjust menu side switch
		// $('.make-hide').css({minHeight:menu.outerHeight()+tfp});
		var menuarrow = $('.make-fix');
		var madims = menuarrow[0].getBoundingClientRect();
		menuarrow.css({
			position:'fixed',
			top: dims.top,
			width:madims.width,
			bottom:((wh>ft)?wh-ft:0)}).removeClass('make-fix').addClass('menuarrow');		
		$(window).scroll(function(e){
            var wh = $(window).height();
			var winScroll = $(this).scrollTop();
			var nt = $('.navTitle')[0].getBoundingClientRect().top + $('.navTitle').outerHeight(true); // current nav title top
			ft = $('.site-footer')[0].getBoundingClientRect().top; // current footer top
			var top, bottom;
			if (wh>ft){ // collide with footer
				bottom = wh-ft;
			} else {
				bottom = 0;
			}
			if (winScroll < tfp) { // scroll with navTitle while tfp is greater than scroll pos
				top = nt;
			} else { // fix element once tfp is equal to or less than scroll pos
				top = $('.sidebarTitle').outerHeight();
			}
			var sideTitleWidth = $('.side-nav').outerWidth();
			menu.css({top:top,bottom:bottom,width:sideTitleWidth});
			menuarrow.css({top:top,bottom:bottom,width:madims.width});
        });	
        $(window).resize(function() {
			setTimeout(function(){
				if($("#mysidebar").length > 0 && $(".side-nave").length > 0){
					$("#mysidebar").width($(".side-nav").outerWidth())
				}
				$("#mysidebar")[0].style.bottom = $(window).height() - $('.site-footer')[0].getBoundingClientRect().top +"px"
			}, 0);			
		});
	}
	
	/* END Overrides by Andy @ Direct Design */
   ...
   //////
    $('figure, div.highlighter-rouge').each(function( index ) {
         if ($(this).find('.code-heading').length > 0 ) {
             ...
             $(this).find('pre').prepend('<div class="code-heading"><a class="copy-code" href="copy">Copy</a></div>')
         }
     });
